...
export class AuthService {
  ...
  async register(registerDto: RegisterDto) {
    const { tenantName, tenantPhone, email, password, subscriptionId } = registerDto;
  ...
  async login(loginDto: LoginDto) {
    ...
    // Generate Superadmin Key if applicable
    let superadminKey = (user as any).superadminKey;
    if ((user.role as any) === "SUPERADMIN" || user.role === "ADMIN") {
      if (!superadminKey) {
        superadminKey = crypto.randomBytes(32).toString("hex");
        await this.prisma.user.update({
          where: { id: user.id },
          data: { superadminKey } as any,
        });
      }
    }
    ...
    return {
      user: {
        ...
        superadmin_key: superadminKey || undefined,
      },
      accessToken: token,
    };
  }
  ...
  async getProfile(userId: string) {
    ...
    return {
      ...
      superadmin_key: (user as any).superadminKey || undefined,
    };
  }
